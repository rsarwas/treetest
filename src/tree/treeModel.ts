...
export class Lock {
	...
	*/
	...
	public run(item: Item, fn: () => Promise): Promise {
...
export class Item {
	...
	private refreshChildren(recursive: boolean, safe: boolean = false, force: boolean = false): Promise {
		...
		var doRefresh = () => {
			...
			const result = childrenPromise.then((elements: any[]) => {
				...
				if (!Array.isArray(elements)) {
					return Promise.wrapError(new Error('Please return an array of children.'));
				}
				...
				if (recursive) {
					return Promise.join(this.mapEachChild((child) => {
	...
	private doRefresh(recursive: boolean, safe: boolean = false): Promise {
		this.doesHaveChildren = this.context.dataSource.hasChildren(this.context.tree, this.element);
		this.height = this._getHeight();
	...
	public refresh(recursive: boolean): Promise {
...
export class TreeModel {
	...
	public refresh(element: any = null, recursive: boolean = true): Promise {
	...
	private resolveUnknownParentChain(element: any): Promise {
