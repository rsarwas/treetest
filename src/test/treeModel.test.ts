...
import lifecycle = require('../common/lifecycle');
import _ = require('../tree/tree');
//import WinJS = require('../common/winjs.base');
import model = require('../tree/treeModel');
import TreeDefaults = require('../tree/treeDefaults');
...
class TestDataSource implements _.IDataSource {
	...
	public getChildren(tree, element): Promise {
		return Promise.as(element.children);
	}
	...
	test('shouldAutoexpand', (done) => {
		...
		const model = new TreeModel({
			dataSource: {
				...
				getChildren: (_, e) => {
					if (e === 'root') { return Promise.wrap(['a', 'b', 'c']); }
					if (e === 'b') { return Promise.wrap(['b1']); }
					return Promise.as([]);
				},
				getParent: (_, e): Promise => { throw new Error('not implemented'); },
...
class DynamicModel implements _.IDataSource {
	...
	public getChildren(tree, element): Promise {
		...
		var result = this.promiseFactory ? this.promiseFactory() : Promise.as(null);
		return result.then(() => {
			this._onDidGetChildren.fire(element);
			return Promise.as(this.data[element]);
		});
	}
...
suite('TreeModel - Dynamic data model', () => {
	...
	test('simultaneously refreshing two disjoint elements should parallelize the refreshes', (done) => {
		...
		model.setInput('root').done(() => {
			...
			dataModel.promiseFactory = () => { return Promise.timeout(0); };
	...
	test('simultaneously recursively refreshing two intersecting elements should concatenate the refreshes - ancestor second', (done) => {
		...
		model.setInput('root').done(() => {
			...
			var p1Complete;
			dataModel.promiseFactory = () => { return new Promise((c) => { p1Complete = c; }); };
			...
			var p2Completes = [];
			dataModel.promiseFactory = () => { return new Promise((c) => { p2Completes.push(c); }); };
...
suite('TreeModel - bugs', () => {
	...
	test('Bug 10566:[tree] build viewlet is broken after some time', (done) => {
		...
		// helpers
		var getGetRootChildren = (children: string[], timeout = 0) => () => Promise.timeout(timeout).then(() => children);
		var getRootChildren = getGetRootChildren(['homer', 'bart', 'lisa', 'marge', 'maggie'], 0);
		var getGetBartChildren = (timeout = 0) => () => Promise.timeout(timeout).then(() => ['milhouse', 'nelson']);
